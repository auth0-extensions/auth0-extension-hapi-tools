...
const validateHookTokenMiddleware = require('../../src/handlers').validateHookToken;
...
tape('validateHookToken should throw error is authorization header is missing', function(t) {
  ...
  validator('/extension').method(req, function(err) {
    t.ok(err);
    t.ok(err.isBoom);
    t.end();
  });
...
tape('validateHookToken validate the token', function(t) {
  ...
  validator('/extension/uninstall').method(req, function(err) {
    t.notOk(err);
    t.end();
  });
});
