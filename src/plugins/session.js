...
module.exports.name = 'dashboardAdminSession';
...
module.exports.register = function(server, options) {
  ...
  server.state(`${nonceKey}_compat`, Object.assign({}, basicCookieAttr, { isSecure: false, isSameSite: false }));
  server.state(`${stateKey}_compat`, Object.assign({}, basicCookieAttr, { isSecure: false, isSameSite: false }));
  ...
  server.route({
    ...
    handler: function(req, reply) {
      ...
      if ((req.state && req.state[stateKey] && findCookie(req.state[stateKey], req.payload.state)) || (req.state && req.state[`${stateKey}_compat`] && findCookie(req.state[`${stateKey}_compat`], req.payload.state))) {
        throw Boom.badRequest('State mismatch');
      }
